...
foreach ( $values as $k => $v ) {
	...
	$color = isset( $colors[ $style ]['normal'][ $v['color'] ] ) ? $colors[ $style ]['normal'][ $v['color'] ] : $v['color'];
	$highlight = isset( $colors[ $style ]['active'][ $v['color'] ] ) ? $colors[ $style ]['active'][ $v['color'] ] : vc_colorCreator($v['color'], - 10 );
...
echo '<div class="' . esc_attr( $css_class ) . '" ' . implode( ' ', $options ) . '>' . $title . '<div class="wpb_wrapper">' . $canvas_html . $legend_html . '</div></div>';